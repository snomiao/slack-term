...
//
// Reads tests/fixtures/raw/*.json, produces tests/fixtures/anon/*.json with
...
function anonId(maps: Maps, kind: "U" | "C" | "T" | "B", real: string): string {
  const m =
    kind === "U"
      ? maps.users
      : kind === "C"
...
function anonName(maps: Maps, kind: "user" | "channel", real: string): string {
  const key = `${kind}:${real}`;
...
function anonText(real: string, seed: number): string {
  if (!real) return real;
...
function walk(val: unknown, maps: Maps, depth = 0): unknown {
  ...
  if (typeof val === "string") {
    ...
    return val
      .replace(/U[A-Z0-9]{6,}/g, (m) => anonId(maps, "U", m))
      .replace(/C[A-Z0-9]{6,}/g, (m) => anonId(maps, "C", m))
      .replace(/T[A-Z0-9]{6,}/g, (m) => anonId(maps, "T", m))
      .replace(/B[A-Z0-9]{6,}/g, (m) => anonId(maps, "B", m))
  ...
  for (const [k, v] of Object.entries(obj)) {
    // Special-case sensitive string fields: replace with anon.
    if (k === "text" && typeof v === "string") {
      out[k] = anonText(v, v.length);
      ...
      out[k] = anonName(maps, looksChannel ? "channel" : "user", v);
...
for (const f of files) {
  ...
  const anon = walk(raw, maps);
  await writeFile(join(ANON, f), JSON.stringify(anon, null, 2));
