...
describe("resolveThreadTs", () => {
  ...
  test("extracts ts from a permalink", () => {
    expect(resolveThreadTs("https://symphoneed.slack.com/archives/C0B47MT6KDM/p1782115333349709"))
      .toBe("1782115333.349709");
  });
  test("prefers thread_ts query param from a permalink", () => {
    expect(resolveThreadTs("https://acme.slack.com/archives/C0B47MT6KDM/p1782115333349800?thread_ts=1782115333.349709"))
      .toBe("1782115333.349709");
  });
  test("throws on a permalink without a timestamp", () => {
    expect(() => resolveThreadTs("https://acme.slack.com/archives/C0B47MT6KDM"))
...
describe("cmdTail", () => {
  ...
  test("--watch-thread merges replies through the poll loop", async () => {
    ...
    try {
      await cmdTail("xoxp-fake", "#symval", { watchThread: "1700000005.000000", interval: 0 }, ac.signal);
    } catch {
      // ignore abort
    } finally {
      spy.mockRestore();
  ...
  test("--watch-thread exits on an invalid ts/permalink", async () => {
    ...
    try {
      await expect(cmdTail("xoxp-fake", "#symval", { watchThread: "garbage", interval: 0 }))
